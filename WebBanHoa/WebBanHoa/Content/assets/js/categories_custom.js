			...
			filterButton.on('click', function () {
				$('.product-grid').isotope({
					filter: function () {
						...
						var priceMin = parseFloat(priceRange.split('-')[0].replace('đ', '').replace(',', '')) || 0;
						var priceMax = parseFloat(priceRange.split('-')[1].replace('đ', '').replace(',', '')) || 3000001;

						var itemPrice = $(this).find('.in_product_price').clone().children().remove().end().text();
						itemPrice = parseFloat(itemPrice.replace('đ', '').replace(',', '')) || 0;
	...
	function initPriceSlider() {
		$("#slider-range").slider({
			...
			slide: function (event, ui) {
				var minText = ui.values[0] === 1 ? "1đ" : ui.values[0] + "đ";
				var maxText = ui.values[1] === 3000001 ? "3000001đ" : ui.values[1] + "đ";
		...
		var initialValue = $("#slider-range").slider("values", 1) === 3000000 ? "3,000,000đ" : $("#slider-range").slider("values", 1) + "đ";
		$("#amount").val("1đ - " + initialValue);
	}
