    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Create(CreateAccountViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    // Add the user to each selected role
        ...
        public async Task<ActionResult> Edit(EditAccountViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Kiểm tra nếu người dùng có quyền Admin, không thêm quyền Admin vào người dùng
                    if (rolesForUser != null && !rolesForUser.Contains("Admin"))
                    {
                        foreach (var role in model.Roles)
                        {
                            // Kiểm tra nếu role là "Admin", không thêm quyền Admin
                            if (role == "Admin")
                                continue;
        ...
        {
            var code = new { Success = false }; // Đặt mặc định không xóa thành công!
            var item = UserManager.FindByName(user);

            // Kiểm tra xem tài khoản cần xóa có phải là tài khoản admin không
            if (item != null && !await UserManager.IsInRoleAsync(id, "Admin"))
            {
                var rolesForUser = UserManager.GetRoles(id);
                if (rolesForUser != null)
                {
